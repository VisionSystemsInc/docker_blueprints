...
# clone MTP
ARG MTP_VERSION="73eb44bb8989158edbc29194f3ca347dc67b2d6b"
ENV MTP_DIR="/mtp" \
    MTP_URL="https://github.com/VisionSystemsInc/MTP"

# COPY ./MTP "${MTP_DIR}"
RUN mkdir -p "${MTP_DIR}"; \
    git clone "${MTP_URL}" "${MTP_DIR}"; \
    git -C "${MTP_DIR}" checkout "${MTP_VERSION}";

# MTP source code adjustments
ENV DCNV3_DIR="/dcnv3"
RUN \
...
RUN \
    ...
    # build wheel
    pip wheel --no-deps --no-build-isolation -w /wheelhouse-tmp -v "${DCNV3_DIR}";
